...
public class ControllerDiagnosticAuthorizationWindow {
    ...
    void onEnterButtonClicked(ActionEvent event) {
        if (!usernameField.getText().equals(Diagnostic.DIAG_NAME) ) {
            Shake shake = new Shake(usernameField);
            shake.playAnimation();
        } else if (!usersurnameField.getText().equals(Diagnostic.DIAG_SURNAME)) {
            Shake shake1 = new Shake(usersurnameField);
            shake1.playAnimation();
        } else if (!passwordField.getText().equals(Diagnostic.DIAG_PASSWORD)) {
            ...
            FXMLLoader loader = new FXMLLoader(getClass().getResource("diagnostic-main-window.fxml"));
            try {
                loader.load();
            } catch (IOException e) {
